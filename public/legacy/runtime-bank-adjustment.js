...
function bankAdjustModel(){
  ...
  let grossShock=18.43e12,recovered=6.08e12,pinch=5.852938441375309e12,bankPinch=4.51e12,fintechPinch=1.34e12;
  ...
  try{if(typeof ratesMoneySummary!=='undefined'){pinch=Math.max(0,-ratesMoneySummary.post.impacto_hogar_total_ampliado);bankPinch=Math.max(0,-ratesMoneySummary.post.impacto_hogar_banco)+Math.max(0,-ratesMoneySummary.post.impacto_hogar_pf);fintechPinch=Math.max(0,-ratesMoneySummary.post.impacto_hogar_fintech)}}catch(e){}
  ...
  return {fiscalSwing,histAvg,taxPp,savingPp,historicalPp,grossShock,recovered,remaining,pinch,bankPinch,fintechPinch,priceFactor,
...
function downloadBankAdjustmentCsv(){
  ...
  lines.push(`hogares,2023-12_2026-07,pinza_financiera_post,${m.pinch.toFixed(0)},ARS jul-2026,"costo del lado hogar; no ganancia bancaria"`);
  lines.push(`hogares,2023-12_2026-07,pinza_sobre_agujero_bruto,${m.pinchGross.toFixed(4)},%,"derivado del dashboard"`);lines.push(`hogares,2023-12_2026-07,pinza_sobre_saldo_no_recuperado,${m.pinchRemaining.toFixed(4)},%,"derivado del dashboard"`);
